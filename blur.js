var blur = function(){
	...
	// Return that shit
	return function(image, width, height, radius){
		
		radius = isNaN(radius) || radius < 1 ? 8 : radius;
		
		var canvas = createImage('canvas', width, height);

		var original = getData(image, width, height);
		var blurred =  stackBlur(original, width, height, radius);
				
		setData(canvas, blurred);
				
		return canvas;

	}
	
	function stackBlur(imageData, width, height, radius){
